"""Download Russell 2000 constituent data from WRDS.

This helper queries the CRSP daily stock file for every permno listed in the
normalized Russell 2000 constituent CSVs.  It pivots the long-format results
...
the index if no official Russell 2000 total-return series is available.
...
def _compute_equal_weight_index(wide: pd.DataFrame) -> pd.DataFrame:
    index_returns = wide.mean(axis=1, skipna=True).to_frame(name="russell2000_equal_weight")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download Russell 2000 data from WRDS")
    parser.add_argument(
        "--permno-csv",
        type=Path,
        default=Path("financial_data/russel2000/constituants/all_permnos.csv"),
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("financial_data/russel2000"),
